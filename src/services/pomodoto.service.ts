...
class PomodoroService {
	...
	async getTodaySession() {
		const response = await axiosWithAuth.get<IPomodotoSessionResponse>(
			`${this.BASE_URL}/today`
		)
	...
	async createSession() {
		const response = await axiosWithAuth.post<IPomodotoSessionResponse>(
			this.BASE_URL
		)
		return response
	}
